...
def test_error_for_invalid_dataset() -> None:
    ...
    try:
        features_train, features_val, target_train, target_val = get_dataset("tests/tem_dirs/test_dataset.csv", 42, 0.2)
...
def test_keep_dir(
    ...
    with runner.isolated_filesystem(temp_dir=tmp_path) as td:
        ...
        result_predict_test = runner.invoke(
                predict, ["--model-path", 'model.joblib',
                            "--dataset-path", "test_df.csv",
                            "--save-submit-path", "submit.csv"],
        )
